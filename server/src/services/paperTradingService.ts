...
import { PaperPosition, PaperState, PaperTrade, SignalRecord } from '../types';
...
const getMinExitAgeMs = (timeframe: string): number => {
  if (timeframe === '15') {
    return 60 * 60 * 1000;
  }
...
const getCooldownMs = (timeframe: string, closeReason?: PaperTrade['closeReason']): number => {
  if (timeframe === '15') {
...
const getSignalAgeMs = (signal: SignalRecord, now: string): number =>
  new Date(now).getTime() - new Date(signal.createdAt).getTime();

const getLatestSignal = (symbol: string, timeframe: string): SignalRecord | undefined =>
  storageService.getSignals().find((item) => item.symbol === symbol && item.timeframe === timeframe);

const higherTimeframeAllowsLong = (signal: SignalRecord): boolean => {
...
const hasRecentTradeCooldown = (state: PaperState, signal: SignalRecord): boolean => {
...
const closePosition = (
  ...
  state.closedTrades = state.closedTrades.slice(0, config.paperMaxClosedTrades);
  state.summary.lastEventAt = closedAt;
};

export class PaperTradingService {
  public getState(): PaperState {
    return storageService.getPaperState();
  }

  public reset(): PaperState {
    ...
    storageService.savePaperState(next);
    return next;
  }

  public processSignal(signal: SignalRecord): PaperState {
    const state = storageService.getPaperState();
    ...
    storageService.savePaperState(state);
    return state;
  }
}
